...
function copyLastBackup(backupConfig, app, manifest, prefix, callback) {
    assert.strictEqual(typeof backupConfig, 'object');
...
function createNewAppBackup(backupConfig, app, manifest, prefix, callback) {
    assert.strictEqual(typeof backupConfig, 'object');
    assert.strictEqual(typeof app, 'object');
    assert(manifest && typeof manifest === 'object');
    assert.strictEqual(typeof prefix, 'string');
...
function backupApp(app, manifest, prefix, callback) {
    ...
    settings.getBackupConfig(function (error, backupConfig) {
        ...
        if (!canBackupApp(app)) {
            ...
            backupFunction = copyLastBackup.bind(null, backupConfig, app, manifest, prefix);
        } else {
            backupFunction = createNewAppBackup.bind(null, backupConfig, app, manifest, prefix);
        }
